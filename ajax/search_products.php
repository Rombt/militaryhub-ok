...
foreach($products as $product) {
    ...
    if(isset($images[$product->id])) {
        $product->image = $okay->design->resize_modifier($images[$product->id], 70, 70);
    }
    $suggestion->price = $okay->money->convert($variants[$product->id][0]->price, $currency->id);
    $suggestion->sku = $variants[$product->id][0]->sku;
    $suggestion->old_price = $variants[$product->id][0]->compare_price;
    $suggestion->test = $variants[$product->id][0];
