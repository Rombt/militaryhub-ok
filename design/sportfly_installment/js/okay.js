...
$(document).on("submit", ".fn_variants", function (e) {
  e.preventDefault();
  var variant, amount, variantName, variantPrice;
  /* Вариант */
...
$(document).on("change", ".fn_variant", function () {
  var selected = $(this).hasClass("variant_select")
...
$(() => {
  if ($("#fn_purchases")) {
    $("#fn_purchases").on("change", ".fn_product_amount input", (e) => {
      ...
      let purchaseNameEl = purchaseItem.find(
...
function sliderInit() {
  if ($(".product__variants_slider").length > 0) {
    $(".product__variants_slider:not(.swiper-initialized)").each(
      function (i, e) {
...
$(document).on("click", ".fn_btn_fast_order", function (e) {
  ...
  let selectedVariant = product.find(
    ".product__variants_item .product_radio:checked",
  );
  let size = selectedVariant.next("label").text();
  let price = selectedVariant
    .find(".product__variants_item .product_radio:checked")
    .data("price");
...
$(document).on("click", ".fn_place_order", function (e) {
  let totalValue = $(
    "#fn_content .fn_cart_step_show .mobile_cart_wrap .purchase_total .total_sum",
  ).data("total");
...
$(document).on("click", ".fn_show_modal", function () {
  ...
  let size = product
    ...
    .text();
  let color = product
    .find(".product_color .fn_select_color:checked")
    .closest("label")
    .attr("title");
...
if (window.location.pathname.indexOf("user") > -1) {
  $(document).on("click", ".fn_referral_add", function (e) {
    ...
    $.post(
      ...
      { ajax: 1, referral: 1, infinity: infinity },
      (r) => {
        ...
        if (r.success) {
          ...
          row
            .find(".order_q .count")
            .text(parseInt(row.find(".order_q .count").text()) + 1);
          if (infinity === 1) {
            btn.remove();
          }
        } else {
...
$(document).on("click", ".fn_live_pagination button", function (a) {
  ...
  $.post(
    e,
    { ajax: 1, page: p, "p[min]": t, "p[max]": send_max },
    function (a) {
      ...
      $(".fn_ajax_wait").remove();
      typeof a.furl !== "undefined"
        ? a.furl.indexOf("stock") === -1
          ? history.pushState(null, null, a.furl)
          : ""
        : "";
      ...
      if (data.success && data.data && data.data.length > 0) {
        ...
        variants.forEach(function (variant, i) {
          if (variant.enabled) {
            ...
            variantRender +=
              ' id="main_product_radio_' + variant.id + "_" + i + '"';
        ...
        if (
          variantFirst &&
          typeof variantFirst.images !== "undefined" &&
          variantFirst.images.length > 0
        ) {
          ...
          for (let i = 0; i < variantFirst.images.length; i++) {
            ...
            galleryTopRender.push(
              '<a href="' +
                image_lg +
                '" data-fancybox="we2" class="swiper-slide fn_ajax_image">' +
                "<picture>" +
                '<img src="' +
                image +
                '" class="fn_img" loading="lazy" alt="" title="">' +
                "</picture>" +
                "</a>",
            );
          ...
          if ($(".gallery-thumbs").length) {
            ...
            document.querySelector(".gallery-top .swiper-wrapper").innerHTML =
              galleryTopRender.join("");
          }
        } else if (
          variantFirst &&
          typeof variantFirst.images !== "undefined" &&
          ...
          const noImage =
            document.querySelector(".gallery-top").dataset.noImage;
        ...
        if (
          !variantActiveCheck &&
          typeof variantActive.compare_price !== "undefined"
        ) {
          $(form)
            .find(".fn_old_price")
            .html(formatNumber(variantActive.compare_price));
        }

        if (
          variantActive.compare_price > variantActive.price &&
          variantActive.price > 0
        ) {
  ...
  $(this).on("change", ".fn_change_variant_cart", function () {
    ...
    ajax.done(function (data) {
      if (data.success && data.data && data.data.length > 0) {
        ...
        if (
          variantFirst &&
          typeof variantFirst.image !== "undefined" &&
          variantFirst.image.length > 0
        ) {
          let productImage = $(self).closest(".fn_product").find(".fn_img");
          let proudctImageName = $(productImage).attr("alt");
          let productImageRender =
            '<img class="fn_img preview_img" src="' +
            variantFirst.image +
            '" alt="' +
            proudctImageName +
            '" title="' +
...
$('input[name="payments_num"]').on("change", function () {
  ...
  let monthPayment = $(input)
    .closest(".installment_flex")
    .find(".fn_monthly_payment");
  let productPrice = $(input)
    .closest(".installment_flex")
    .data("price")
    .toString()
    .replace(/\s/g, "");
