...
class ReportStat extends Okay {
    ...
    private function make_filter($filter = array()) {
        ...
        if (isset($filter['user_filter'])) {
            switch ($filter['user_filter']) {
                case 'no_users': {
                    $user_filter = 'AND (o.user_id IS NULL OR o.user_id = 0)';
                    break;
                }
                case 'only_users': {
                    $user_filter = 'AND o.user_id';
                    break;
                }
            }
        }
        ...
        if (isset($filter['group'])) {
            $group_filter = $this->db->placehold(" AND o.user_id = (SELECT u.id FROM __users u INNER JOIN __groups g ON g.id = u.group_id WHERE u.group_id = ?)", (int)$filter['group']);
        }

        if (isset($filter['referral_filter'])) {
            $referral_filter = $this->db->placehold(' AND o.referral_id = ?', intval($filter['referral_filter']));
        }

        if (isset($filter['paid_filter'])) {
            switch ($filter['paid_filter']) {
                case 'paid': {
                    $paid_filter = 'AND o.paid = 1';
                    break;
                }
                case 'no_paid': {
                    $paid_filter = 'AND o.paid = 0';
                    break;
                }
            }
        }

        return "$status_filter $date_filter $period_filter $group_filter $user_filter $referral_filter $paid_filter";
    }
