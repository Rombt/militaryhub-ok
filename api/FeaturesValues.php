...
class FeaturesValues extends Okay
{
    ...
    public function get_features_values($filter = array(), $count = false)
    {
        ...
        if (isset($filter['yandex'])) {
            $yandex_filter = $this->db->placehold(
                "AND `f`.`yandex`=? AND `pf`.`product_id` IN (
                SELECT 
                    distinct(p.id)
                FROM __variants v 
        ...
        if (!empty($filter['feed'])) {
            switch ($filter['feed']) {
            case 'rozetka':
                $feed_filter = 'AND v.feed_rozetka = 1';
                break;
            case 'prom':
                $feed_filter = 'AND v.feed_prom = 1';
                break;
            case 'rss':
                $feed_filter = 'AND v.feed_rss = 1';
                break;
            default:
                $feed_filter = 'AND v.feed = 1';
            }
            $feed_filter = $this->db->placehold(
        ...
        if(isset($filter['features'])) {
            ...
            foreach($filter['features'] as $feature_id=>$value) {
                $features_filter_array[] = $this->db->placehold(
                    "(`fv`.`feature_id`=? OR `pf`.`product_id` in (SELECT `pf`.`product_id` FROM `__products_features_values` AS `pf`
        ...
        if ($count) {
            ...
            $select = $this->db->placehold(
                ...
                MAX(`c`.`rozetka_exclude`) AS `rozetka_exclude`,        /* //!! rmbt для rozetka_new.php */
                MAX(`c`.`id`) AS `category_site_id`,                  /* //!! rmbt для rozetka_new.php  для отладки!!!*/
                MAX(`c`.`name`) AS `category_site_name`,                  /* //!! rmbt для rozetka_new.php  для отладки!!!*/
        ...
        $query = $this->db->placehold(
            ...
            LEFT JOIN `__categories` AS `c` ON `c`.`rozetka_category_value_id`=`fv`.`id`    /* //!! rmbt для rozetka_new.php */
    ...
    /*Метод возвращает ID всех значений свойств товаров*/
    public function get_products_values_id($product_id)
    {
        ...
        $featuresIDPrio = array_filter(
            array_map('trim', (array)$this->settings->feature_id),
            fn($v) => $v !== '' && $v !== null
        );
        $featuresIDPrioString = implode(',', $featuresIDPrio);

        $query = $this->db->placehold(
            "SELECT pfv.product_id, pfv.value_id, fv.feature_id
                FROM __products_features_values pfv
                INNER JOIN __features_values fv ON fv.id = pfv.value_id
                INNER JOIN __features f ON f.id = fv.feature_id
                WHERE pfv.product_id IN (?@)
                ORDER BY f.position, fv.feature_id, FIELD(fv.feature_id, ".$featuresIDPrioString.") DESC", (array)$product_id
        );
