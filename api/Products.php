...
class Products extends Okay
{
    ...
    public function get_products($filter = array(), $count = false)
    {
        ...
        $where    = '1';
        $group_by = '';
        $order    = 'p.main_order DESC, p.position DESC';
        ...
        //!!
        if (!empty($filter['exclude_category_id'])) {
            $where .= $this->db->placehold(' AND p.id NOT IN (SELECT pc.product_id FROM __products_categories pc WHERE pc.category_id IN(?@))', (array)$filter['exclude_category_id']);
        }

        //!!
        if (!empty($filter['before_date'])) {
            $where .= $this->db->placehold(' AND p.created < ?', $filter['before_date']);
        }
        ...
        if (isset($filter['no_discounted'])) {
            $where .= $this->db->placehold(' AND ( SELECT 1 FROM __variants pv WHERE pv.product_id = p.id AND (pv.compare_price = 0 OR pv.compare_price IS NULL) LIMIT 1) ');
        }
        ...
        if (isset($filter['max_stock'])) {
            // $where .= $this->db->placehold(' AND (SELECT count(*)>0 FROM __variants pv WHERE pv.product_id=p.id AND pv.stock IS NOT NULL AND pv.stock BETWEEN 1 AND ? LIMIT 1) = 1', intval($filter['max_stock']));
            $where .= $this->db->placehold(' AND (SELECT sum(pv.stock) FROM __variants pv WHERE pv.product_id=p.id) BETWEEN 1 AND ?', intval($filter['max_stock']));
        }
        ...
        if (isset($filter['feed_rozetka'])) {
            $joins .= $this->db->placehold(' inner join __variants v on v.product_id=p.id and v.feed_rozetka=?', intval($filter['feed_rozetka']));
        }

        if (isset($filter['feed_epicentr'])) {
            $joins .= $this->db->placehold(' inner join __variants v on v.product_id=p.id and v.feed_epicentr=?', intval($filter['feed_epicentr']));
        }

        if (isset($filter['feed_prom'])) {
            $joins .= $this->db->placehold(' inner join __variants v on v.product_id=p.id and v.feed_prom=?', intval($filter['feed_prom']));
        }

        if (isset($filter['feed_rss'])) {
            $joins .= $this->db->placehold(' inner join __variants v on v.product_id=p.id and v.feed_rss=? AND (v.stock > 0 OR v.stock is NULL)', intval($filter['feed_rss']));
        }
        ...
        if (!empty($filter['from_date']) || !empty($filter['to_date'])) {
            ...
            $where .= $this->db->placehold(" AND (p.created BETWEEN ? AND ?)", $from, $to);
        }
        ...
        $sort        = new stdClass();
        $sort->price = $this->db->placehold('IF (pv.price > 0, 1, 0) DESC');
        $sort->stock = $this->db->placehold('IF((SELECT SUM(IFNULL(pv.stock, ?)) FROM __variants pv WHERE p.id = pv.product_id), 1, 0) DESC', $this->settings->max_order_amount);
    ...
    public function duplicate_product($id)
    {
        ...
        // Очищаем url и выключаем
        $this->db->query('UPDATE __products SET url="", visible=0 WHERE id=?', $new_id);
        ...
        foreach ($variants as $variant) {
            ...
            $variant->external_id   = '';
            $variant->feed          = 0;
            $variant->feed_rozetka  = 0;
            $variant->feed_epicentr = 0;
            $variant->feed_prom     = 0;
            $variant->feed_rss      = 0;
    ...
    public function get_images($filter = array())
    {
        ...
        if (!empty($filter['variant_id'])) {
            $where .= $this->db->placehold(' AND i.variant_id in(?@) ', (array)$filter['variant_id']);
        } else {
            $where .= $this->db->placehold(' AND i.variant_id = 0');
        }

        if (!empty($filter['id'])) {
            $where .= $this->db->placehold(' AND i.id in(?@) ', (array)$filter['id']);
        }
    ...
    public function add_image($product_id, $filename, $name = '', $variant_id = 0)
    {
        $query = $this->db->placehold("SELECT id FROM __images WHERE product_id=? AND variant_id=? AND filename=?", $product_id, $variant_id, $filename);
        $this->db->query($query);
        $id = $this->db->result('id');
        if (empty($id)) {
            $query = $this->db->placehold("INSERT INTO __images SET product_id=?, variant_id=?, filename=?", $product_id, $variant_id, $filename);
            $this->db->query($query);
            $id    = $this->db->insert_id();
    ...
    public function tiny_products(&$products, $filter = array())
    {
        $products_ids         = array_keys($products);
        $brands_ids           = array();
        $images_ids           = array();
        $products_v_positions = [];
        $colors_ids           = [];
        $sizes_ids            = [];
        ...
        foreach ($products as &$product) {
            ...
            $products_v_positions[$product->id] = [];
        }

        $filter               = array_diff_key($filter, array_flip(['sort', 'page', 'limit']));
        $filter['product_id'] = $products_ids;
        // $filter['sort'] = 'position';
        $variants = $this->variants->get_variants($filter);
        ...
        foreach ($variants as $variant) {
            ...
            $products_v_positions[$variant->product_id][$variant->position] = $variant->id;

            $color_translit              = $this->translit_alpha($variant->color);
            $colors_ids[$color_translit] = $variant->color;
            if (!in_array($color_translit, array_keys($products[$variant->product_id]->colors))) {
                $products[$variant->product_id]->colors[$color_translit] = $variant;
            }

            $size_translit             = $this->translit_alpha($variant->size);
            $sizes_ids[$size_translit] = $variant->size;
            if (!isset($products[$variant->product_id]->sizes[$color_translit])) {
                $products[$variant->product_id]->sizes[$color_translit] = [];
            }
            if (!isset($products[$variant->product_id]->sizes[$color_translit][$size_translit])) {
                $products[$variant->product_id]->sizes[$color_translit][$size_translit] = $variant;
            }
        }

        $images = $this->get_images(array('product_id' => $products_ids));
        foreach ($images as $image) {
            if (!isset($products[$image->product_id]->images)) {
                $products[$image->product_id]->images = [];
            }
        ...
        if ($variants_images = $this->variants->get_images(array('product_id' => $products_ids, 'color' => $colors_ids, 'group_by' => 'variant'))) {
            foreach ($variants_images as $image) {
                foreach ($products[$image->product_id]->variants as &$variant) {
                    if (($variant->color == $products[$image->product_id]->variants[$image->variant_id]->color)/* && empty($variant->images)*/) {
        ...
        foreach ($products as &$product) {
            ...
            foreach ($product->variants as $variant) {
                ...
                if (isset($product->colors[$color_id]) && !empty($variant->images) && $variant->color == $product->colors[$color_id]->color) {
            ...
            if (!empty($product->variants) && !isset($product->variant)) {
    ...
    public function get_productReviewsCount($id)
    {
        if (empty($id)) {
            return false;
        }

        $filter = $this->db->placehold('AND comm.object_id =? AND comm.type =?', (int)$id, 'product');

        $query = "SELECT COUNT(*) as count
            FROM __comments AS comm
            WHERE 
                1 
                $filter
        ";
        ...
        $this->db->query($query);
        $productReviewsCount = $this->db->result();
        return $productReviewsCount ?: 0;
    }


    public function get_metabyuri($uri)
    {
        $uri   = substr($uri, 1);
        $where = $this->db->placehold('AND mc.url=? ', $uri);
    ...
    public function get_seocommentbyuri($uri)
    {
        $uri   = substr($uri, 1);
        $where = $this->db->placehold('AND sc.url=? ', $uri);
    ...
    public function get_fastFiltersByUri($uri)
    {
        $uri   = substr($uri, 1);
        $where = $this->db->placehold('AND ff.url=? ', $uri);
