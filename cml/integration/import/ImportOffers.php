...
class ImportOffers extends Import
{
    ...
    private function import_variant($xml_variant) {
        ...
        if (isset($xml_variant->Цены->Цена->СтараяЦена)) {
            $variant->compare_price = (float)$xml_variant->Цены->Цена->СтараяЦена;
        } else {
        ...
        if (!empty($xml_variant->Цены->Цена->Валюта)) {
            ...
            $currency_sign = (string)mb_strtolower($xml_variant->Цены->Цена->Валюта);

            // Ищем валюту по коду или обозначению
            if (isset($this->currencies_by_sign[$currency_sign])) {
                $variant_currency = $this->currencies_by_sign[$currency_sign];
            } elseif (isset($this->currencies_by_code[$currency_code])) {
            ...
            if ($variant_currency && $variant_currency->rate_from>0 && $variant_currency->rate_to>0 && !$this->is_multi_currency) {
                ...
                $variant->compare_price = floatval($variant->compare_price)*$variant_currency->rate_to/$variant_currency->rate_from;
            }
        }
