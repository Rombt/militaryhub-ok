...
class LoginView extends View {
    ...
    public function fetch() {
        ...
        elseif($this->request->method('post') && $this->request->post('login')) {
            ...
            $promo_group_id = intval($this->settings->registration_promo_group);
            $default_group_id = intval($this->settings->registration_group);

            if ($promo_group_id > 0) {
                $query = $this->db->placehold("UPDATE __users SET group_id = ? WHERE group_id = ? AND created < (NOW() - INTERVAL 30 DAY)", $default_group_id,  $promo_group_id);
                $this->db->query($query);
            }

            $this->design->assign('phone', $phone);
            if($user_id = $this->users->check_password($phone, $password)) {
