...
class Image extends Okay
{
    ...
    {
        list($source_file, $width, $height, $set_watermark, $crop_params, $pseudoWebp) = $this->get_resize_params($filename);
        ...
        // якщо вже є готовий файл — одразу повертаємо
        if (is_file($destination) && filesize($destination) > 0 && !$pseudoWebp) {
            return $destination;
        }
        if ($pseudoWebp) {
            $webpDest = $destination . '.webp';
        ...
        // lock на конкретний файл ресайзу
        $lockKey = sha1($destination . '|' . ($pseudoWebp ? 'webp' : ''));
        $lockFp = @fopen(sys_get_temp_dir() . '/okay_resize_' . $lockKey . '.lock', 'c');

        if ($lockFp) {
            flock($lockFp, LOCK_EX);
            // після лока ще раз перевіряємо (можливо вже згенерували)
            clearstatcache(true, $destination);

            if (!$pseudoWebp && is_file($destination) && filesize($destination) > 0) {
        ...
        // пишемо в tmp → rename (атомарно)
        $tmp = $destination . '.tmp.' . getmypid();

        if ($this->config->resize_library == 'gregwar_image') {
            $this->image_gregwar_image($originals_dir . $original_file, $tmp, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y, $crop_params);
        } elseif ($this->config->resize_library == 'imagick' && class_exists('Imagick')) {
            $this->image_constrain_imagick($originals_dir . $original_file, $tmp, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y);
        } else {
            $this->image_constrain_gd($originals_dir . $original_file, $tmp, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y);
        }

        // якщо tmp не створився або 0 байт — прибираємо і виходимо
        ...
        // атомарно підміняємо кінцевий файл
        @rename($tmp, $destination);

        // pseudoWebp — теж через tmp
        if ($pseudoWebp) {
            $webpDest = $destination . '.webp';
            $webpTmp  = $webpDest . '.tmp.' . getmypid();

            try {
                WebPConvert::convert($destination, $webpTmp);
            } catch (\Throwable $e) {
                @unlink($webpTmp);
            }

            clearstatcache(true, $webpTmp);
            if (is_file($webpTmp) && filesize($webpTmp) > 0) {
                @rename($webpTmp, $webpDest);
                $destination = $webpDest;
            } else {
                @unlink($webpTmp);
                // fallback: хоча б jpg/png віддамо
    ...
    public function add_resize_params($filename, $width = 0, $height = 0, $set_watermark = false, $crop_params = array(), $webp = false)
    {
        ...
        $resized_filename .= '.' . $ext;
        if ($webp) {
            $resized_filename .= '.webp';
        }
