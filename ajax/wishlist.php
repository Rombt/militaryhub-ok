...
if (!count($products_ids)) {
    ...
    setcookie('wished_products', '', time()-3600, '/', $okay->config->root_host, false, true);
} else {
    setcookie('wished_products', implode(',', $products_ids), time()+30*24*3600, '/', $okay->config->root_host, false, true);
}
if ($user_id) {
    $wp_ids = implode(',', $products_ids);
    if (!empty($wp_ids)) {
        $okay->wishlist->update($user_id, $wp_ids);
    } else {
        $okay->wishlist->clear($user_id);
    }
}
