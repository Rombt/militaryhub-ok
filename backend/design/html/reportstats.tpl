                ...
                <div class="boxed_sorting toggle_body_wrap off fn_card">
                    <div class="row">
                        ...
                        <div class="col-xs-12 mb-1">
                            ...
                            <div class="col-md-3 col-lg-3 col-sm-12 mb-1">
                                <select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
                                    <option {if !$smarty.get.group}selected{/if} value="{url group=null}">{$btr->reportstats_all_groups|escape}</option>
                                    {foreach $groups as $group}
                                        <option {if $group->id == $smarty.get.group}selected{/if} value="{url group=$group->id}">{$group->name|escape}</option>
                                    {/foreach}
                                </select>
                            </div>
                            <div class="col-md-3 col-lg-3 col-sm-12 mb-1">
                                <select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
                                    <option {if !$user_filter}selected{/if} value="{url user_filter=null}">{$btr->reportstats_buyers|escape}</option>
                                    <option {if $user_filter == no_users}selected{/if} value="{url user_filter=no_users group=null}">{$btr->reportstats_no_users|escape}</option>
                                    <option {if $user_filter == only_users}selected{/if} value="{url user_filter=only_users}">{$btr->reportstats_only_users|escape}</option>
                                </select>
                            </div>
                            <div class="col-md-3 col-lg-3 col-sm-12 mb-1">
                                <select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
                                    <option {if !$smarty.get.referral}selected{/if} value="{url referral=null}">{$btr->reportstats_referrals|escape}</option>
                                    {foreach $referrals as $referral}
                                        <option {if $smarty.get.referral == $referral->id}selected{/if} value="{url referral=$referral->id}">{$referral->name|escape}</option>
                                    {/foreach}
                                </select>
                            </div>
                            <div class="col-md-3 col-lg-3 col-sm-12 mb-1">
                                <select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
                                    <option {if !$paid_filter}selected{/if} value="{url paid_filter=null}">{$btr->reportstats_all_orders|escape}</option>
                                    <option {if $paid_filter == paid}selected{/if} value="{url paid_filter=paid group=null}">{$btr->reportstats_paid|escape}</option>
                                    <option {if $paid_filter == no_paid}selected{/if} value="{url paid_filter=no_paid}">{$btr->reportstats_no_paid|escape}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-xs-12 mb-1">
    ...
    <form method="post" class="fn_form_list">
        ...
        <div class="main_list products_list fn_sort_list">
            ...
            <div class="main_list_head">
                <div class="main_list_heading main_list_reportstats_categories">{$btr->general_category|escape}</div>
                <div class="main_list_heading main_list_reportstats_products">{$btr->general_name|escape}</div>
                <div class="main_list_heading main_list_reportstats_total">{$btr->general_sales_amount|escape}</div>
                <div class="main_list_heading main_list_reportstats_setting">{$btr->general_amt|escape}</div>
                <div class="main_list_heading main_list_reportstats_products">{$btr->order_buyer|escape}</div>
                <div class="main_list_heading main_list_reportstats_products">{$btr->email_order_phone|escape}</div>
            </div>
                ...
                {foreach $report_stat_purchases as $purchase}
                    ...
                    <div class="main_list_body_item">
                        <div class="main_list_row ">
                            <div class="main_list_boding main_list_reportstats_categories">
                                {foreach $purchase->category->path as $c}
                                    {$c->name}/
                                {/foreach}
                            </div>
                            <div class="main_list_boding main_list_reportstats_products">
                            ...
                            <div class="main_list_boding main_list_reportstats_total">
                                {$purchase->sum_price} {$currency->sign|escape}
                            </div>

                            <div class="main_list_reportstats_setting">
                                {$purchase->amount} {if $purchase->units}{$purchase->units|escape}{else}{$settings->units}{/if}
                            </div>

                            <div class="main_list_boding main_list_reportstats_products">
                                {if $purchase->user_id}
                                    <a href="{url module=UserAdmin id=$purchase->user_id return=$smarty.session.REQUEST_URI}" target="_blank">{$purchase->name}</a>
                                {else}
                                    {$purchase->name}
                                {/if}
                            </div>
...
<script>
    ...
    {/if}
    {if $referral_filter}
    var referral = '{$referral_filter}';
    {/if}
    {if $paid_filter}
    var paid = '{$paid_filter}';
    {/if}
    {if $sort_prod}
    ...
    <script type="text/javascript">
        $(function() {
            ...
            function do_export(page) {
                ...
                referral =  typeof(referral) != 'undefined' ? referral : 0;
