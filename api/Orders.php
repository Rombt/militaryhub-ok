...
class Orders extends Okay {
    ...
    public function get_order($id) {
        ...
        return $this->get_orders($filter, false, true);
    }

    /*Выборка всех заказов*/
    public function get_orders($filter = array(), $count = false, $one = false) {
        // По умолчанию
        $limit = 100;
        $page = 1;
        $joins = '';
        ...
        $select = "o.id, 
            ...
            o.store_id,
            o.referral_id,
            o.ga_ecommerce_sended,
            o.bonuses,
            o.paid_bonuses,
            os.color as status_color,
            os.status_1c as status_1c";
        ...
        if (isset($filter['id'])) {
            if (is_int($filter['id'])) {
                $where .= $this->db->placehold(' AND o.id = ?', intval($filter['id']));
            } elseif (!empty($filter['id']) && is_array($filter['id'])) {
                $where .= $this->db->placehold(' AND o.id in (?@)', (array)$filter['id']);
            } else {
                $where .= $this->db->placehold(' AND o.url = ?', (string)$filter['id']);
            }
        }
        ...
        if (isset($filter['to_id'])) {
            $where .= $this->db->placehold(" AND o.id < ?", intval($filter['to_id']));
        }
    ...
    public function update_order($id, $order) {
        ...
        if (!empty($order->status_id) || !empty($order->referral_id)) {
            $old_order = $this->get_order(intval($id));
            $old_order = $this->set_order_bonuses($old_order, $order);
            $order->bonuses = $old_order->bonuses;
        }
    ...
    private function set_order_bonuses($order, $old_order) {
        if (!empty($order->user_id)) {
            $order->bonuses = empty($old_order->bonuses) ? 0 : $old_order->bonuses;

            $users_bonuses_on = (boolean)$this->settings->users_bonuses_on;
            if ($users_bonuses_on === true && empty((float)$old_order->paid_bonuses)) {
                $purchases = $this->get_purchases(array('order_id' => $order->id));
                $total_bonuses_price = array_sum(array_map(function ($purchase) { return empty($purchase->bonuses) ? 0 : $purchase->price; }, $purchases));
                if (!empty($old_order->status_id)) {
                    $status = $this->orderstatus->get_status(array('status' => intval($old_order->status_id)));
                    if (!empty($status) && !in_array(reset($status)->status_1c, ['new', 'to_delete'])) {
                        if (!($this->orders->count_orders(array('to_id' => $order->id, 'user_id' => $order->user_id)))) {
                            $bonuses = floatval($this->settings->orders_first_bonuses);
                            $order->bonuses += $bonuses;
                        }

                        $order_min_total_price = (int)$this->settings->orders_total_price_bonuses;
                        if ($total_bonuses_price >= $order_min_total_price) {
                            while (!empty($purchases)) {
                                $purchase = array_shift($purchases);
                                $order->bonuses += ($this->money->convert($purchase->bonuses, null, false) * $purchase->amount);
                            }
                        }
                    }
                }
                ...
                } elseif (!empty($old_order->referral_id)) {
                    $referral_id = $old_order->referral_id;
                }

                if (!empty($referral_id) && $referral = $this->ref->get_item(intval($referral_id))) {
                    $referral_bonuses = 0.00;
                    $referrer_bonuses = 0.00;
                    if ($referral->type == 'absolute') {
                        $referral_bonuses = $referral->value;
                        $referrer_bonuses = $referral->value_referrer;
                    } else {
                        $referral_bonuses = round($total_bonuses_price * ($referral->value / 100), 2);
                        $referrer_bonuses = round($total_bonuses_price * ($referral->value_referrer / 100), 2);
                    }
    
                    $order->bonuses += $referrer_bonuses;
    
                    if (!empty($referral->user_id) && $referral_bonuses > 0) {
                        $this->users->add_user_bonus($referral->user_id, $referral->id, $referral_bonuses, "referral_{$order->id}");
                    }
                }
            }

            if ($order->bonuses > 0) {
                $this->users->add_user_bonus($order->user_id, $order->id, $order->bonuses);
            }
        }
    ...
    public function delete_order($id) {
        if (!empty($id)) {
            ...
            $query = $this->db->placehold("DELETE FROM __users_bonuses WHERE object_id = ? AND type LIKE ?", $id, 'order%');
            $this->db->query($query);
        }
    }
    ...
    public function get_purchases($filter = array()) {
        ...
        if (!empty($filter['order_id'])) {
            $where .= $this->db->placehold(' AND p.order_id in(?@)', (array)$filter['order_id']);
        }

        // (SELECT ROUND(IF(bonuses > 0, bonuses, IFNULL(v.price, 0)*?/100), 2) FROM __products WHERE id = p.product_id LIMIT 1) as bonuses
        $query = $this->db->placehold("SELECT
            p.*,
            pr.main_category_id,
            ROUND(IF(pr.bonuses > 0, pr.bonuses, IFNULL(v.price, 0)*?/100), 2) as bonuses
        FROM __purchases p
        LEFT JOIN __variants v ON v.id = p.variant_id
        LEFT JOIN __products pr ON pr.id = p.product_id
        WHERE $where
        ORDER BY p.id", (float)$this->settings->products_bonuses);
        $this->db->query($query);
        ...
        $purchases = array_map(function ($purchase) use ($restricted_bonuses_category) {
            $category = $this->categories->get_category(intval($purchase->main_category_id));
            $purchase->bonuses *= $restricted_bonuses_category && in_array($category->id, $restricted_bonuses_category->children) ? 0 : 1;
            return $purchase;
        }, $purchases);
        return $purchases;
    }
    ...
    public function add_purchase($purchase) {
        ...
        if (!isset($purchase->price) && !empty($variant)) {
            $purchase->price = round($variant->price);
        }
    ...
    private function update_total_price($order_id) {
        ...
        $query = $this->db->placehold("UPDATE __orders o SET o.total_price=IFNULL((SELECT SUM(p.price*p.amount)*(100-o.discount)/100 FROM __purchases p WHERE p.order_id=o.id), 0)-o.paid_bonuses+o.delivery_price*(1-IFNULL(o.separate_delivery, 0))-o.coupon_discount WHERE o.id=? LIMIT 1", $order->id);
        $this->db->query($query);
        return $order->id;
    }
