...
class ProductsView extends View
{
    ...
    private $meta_change              = array('h1' => '', 'title' => '', 'description' => '');
    private $seo_comment              = array('body' => '');
    ...
    public function __construct()
    {
        ...
        //!!
        $this->clearance_sale = $_GET['clearance-sale'];

        if ($category_url = $this->request->get('category', 'string')) {
            $this->category = $this->categories->get_category((string)$category_url);

            if ((empty($this->category) || (!$this->category->visible && empty($_SESSION['admin']))) && $this->clearance_sale !== '1') { //!! добавил  && $this->clearance_sale !== '1'
        ...
        foreach ($this->uri_array as $k => $v) {
            ...
            if (!$k && $brand = $this->brands->get_brand((string)$v)) {
                ...
                if (in_array($this->page->url, array('all-products', 'discounted', 'bestsellers', 'last-products'))
                ...
                switch ($param_name) {
                    ...
                    case 'available':
                    {
                        $_GET['available'] = strval($param_values);
                        if (!in_array($_GET['available'], array('instock'))) {
                            $this->is_wrong_params = 1;
                        }
                        break;
                    }
                    ...
                    case 'sale':
                    {
                        $sale_category_url = ltrim(strstr($v, '-'), '-');
                        $query             = $this->db->placehold("SELECT c.id FROM __categories c WHERE url=?", $sale_category_url);
                        $this->db->query($query);
                        $sale_category_id         = $this->db->results('id');
                        $_GET['sale_category_id'] = $sale_category_id;
                        break;
                    }
                    default:
                    {
    ...
    public function filter_chpu_url($params, &$smarty)
    {
        ...
        foreach ($this->uri_array as $k => $v) {
            ...
            switch ($param_name) {
                ...
                case 'available':
                {
                    $result_array['available'] = strval($param_values);
                    break;
                }
                case 'page':
                {
        ...
        /*фільтр по наявності*/
        if (!empty($result_array['available'])) {
            $result_string .= '/available-' . $result_array['available'];
        }
    ...
    private function filter_chpu_parse_url()
    {
        ...
        preg_match("~^$this->subdir(/?$this->lang_label)?/?(catalog|all-products|brands|discounted|bestsellers|last-products|clearance-sale)/?~", $uri['path'], $this->catalog_type);        //!! добавил |clearance-sale
        $this->catalog_type = $this->catalog_type[2];
        //убираем модификатор каталога
        $uri             = preg_replace("~^$this->subdir(/?$this->lang_label)?/?(catalog|all-products|brands|discounted|bestsellers|last-products|clearance-sale)/?~", '', $uri['path']);        //!! добавил |clearance-sale
    ...
    public function fetch()
    {
        ...
        if (!$ajax = $this->request->get('ajax', 'boolean')) {
            $ajax = $this->request->post('ajax', 'boolean');
        }
        ...
        if (!empty($prices['current'])) {
            $prices['current'] = [
                'min' => isset($prices['current']['min']) ? $prices['current']['min'] : 0,
                'max' => isset($prices['current']['max']) ? max((isset($prices['current']['max']) ? $prices['current']['max'] : 0), $prices['current']['max']) : PHP_INT_MAX,
            ];
        }
        ...
        if (!empty($mode)) {
            if ($mode == 'bestsellers') {
                ...
                $filter['discounted'] = 1;
            } elseif ($mode == 'last-products') {
                $filter['max_stock'] = 3;

                //!!
                $filter['exclude_category_id'] = [125, 250, 252, 253, 254, 255, 256, 257, 258, 259, 260, 261, 262, 266, 267, 268, 269, 270,];
            }
        }
        ...
        // Наличие товаров
        if ($available = $this->request->get('available', 'string')) {
            switch ($available) {
                case 'instock':
                {
                    $filter['in_stock'] = 1;
        ...
        $this->design->assign('available', $available);
        ...
        if (!empty($this->category)) {
            ...
            foreach ($features_values as $feature_value) {
                ...
                if (in_array($feature_value->feature_id, $this->settings->feature_id)) {
                    if (!isset($product_values_aliases[$feature_value->feature_id])) {
                        $product_values_aliases[$feature_value->feature_id] = [];
                    }
                    $product_values_aliases[$feature_value->feature_id][$feature_value->translit] = $feature_value->value;
                }
            }
        ...
        //!!
        if (!$this->clearance_sale) {
            $this->design->assign('other_filters', $other_filters);
        } else {
            $filter['before_date'] = '2022-01-01';
            $filter['category_id'] = $_GET['sale_category_id'];
            $filter['in_stock']    = 1;
        }
        ...
        //Зміна кількості товарів на сторінці
        if ($this->request->post("show_count", 'int')) {
            $items_per_page                = $this->request->post("show_count", 'int');
            $_SESSION['products_per_page'] = $items_per_page;
        } elseif (isset($_SESSION['products_per_page'])) {
            $items_per_page = $_SESSION['products_per_page'];
        }
        $this->design->assign('products_per_page', $items_per_page);

        // Текущая страница в постраничном выводе
        if (!($page = $this->request->get('page')) && $ajax) {
        ...
        foreach ($this->products->get_products($filter) as $p) {
            $products[$p->id]                    = $p;
            $products[$p->id]->reviewsCount      = $this->products->get_productReviewsCount($p->id);
            $products[$p->id]->reviewsCountTitle = $this->products->declOfNum($products[$p->id]->reviewsCount, $this->translations->get_translation('reviews_count')->{"lang_$this->lang_label"});
            // $products[$p->id]->all_img = $this->products->get_images(array('product_id'=>$p->id));

            //!!
            if ($this->clearance_sale) {
                $arr_categories_id[] = $p->main_category_id;
            }
        }

        //!!
        $arr_categories_id = array_values(array_unique($arr_categories_id));
        if ($this->clearance_sale) {
            $clearance_sale_categories = $this->categories->get_categories(array(
                'id'                  => $arr_categories_id,
                'category_visible'    => 1,
                'level_depth'         => [1, 2, 3],
                'in_stock'            => 1,
                'exclude_category_id' => [204, 205, 206, 207, 208, 209, 210, 211, 212, 213, 214, 215, 216, 217, 229, 252],
            ));

            $arr_group_subcategories = [
                'for_men'      => [
                    'title'         => 'Для чоловіків',
                    'id'            => [133, 134, 140, 135, 136, 137, 138, 139, 189, 141, 142, 143, 144, 145, 146,],
                    'subcategories' => [],
                ],
                'for_women'    => [
                    'title'         => 'Для жінок',
                    'id'            => [147, 148, 149, 150, 151, 152, 153, 155, 156, 157, 158, 159, 160, 161,],
                    'subcategories' => [],
                ],
                'for_children' => [
                    'title'         => 'Для дітей',
                    'id'            => [162, 163, 180, 164, 167, 172, 181, 186,],
                    'subcategories' => [],
                ],
                'other'        => [
                    'title'         => 'Інше',
            ...
            foreach ($clearance_sale_categories as $category) {
                foreach ($category->subcategories as $i => $subcategories) {
                    $filter['category_id'] = $subcategories->id;
                    $count_products        = $this->products->get_products($filter, true);
                    if ($count_products == '0') {
                        unset($category->subcategories[$i]);
                        continue;
                    }

                    if (in_array($subcategories->id, $arr_group_subcategories['for_men']['id'])) {
                        $arr_group_subcategories['for_men']['subcategories'][] = $subcategories;
                    } elseif (in_array($subcategories->id, $arr_group_subcategories['for_women']['id'])) {
                        $arr_group_subcategories['for_women']['subcategories'][] = $subcategories;
                    } elseif (in_array($subcategories->id, $arr_group_subcategories['for_children']['id'])) {
                        $arr_group_subcategories['for_children']['subcategories'][] = $subcategories;
                    } else {
                        $arr_group_subcategories['other']['subcategories'][] = $subcategories;
                    }
                }
            }

            $this->design->assign('arr_group_subcategories', $arr_group_subcategories);
            $this->design->assign('clearance_sale_h1', 'Розпродаж попередніх колекцій');
        }


        // Если искали товар и найден ровно один - перенаправляем на него
        if (!empty($keyword) && $products_count == 1 && !$ajax) {
            header('Location: ' . $this->config->root_url . '/' . $this->lang_link . 'products/' . $p->url);
        }

        if (!empty($products)) {
            unset($filter['id']);
            $this->products->tiny_products($products, $filter);
            $this->design->assign('products', $products);

            $label       = $this->translations->get_translation('next', true);
            $button_text = $label->{"lang_{$this->language->label}"};
            $count       = ($pages_num == ($current_page + 1)) ? ceil($products_count - ($current_page * $items_per_page)) : $items_per_page;
            $arr         = [
                '_count_'    => $count,
                '_products_' => $this->design->plural_modifier($count, $this->translations->products_item, $this->translations->products_all_items, $this->translations->products_of_items),
            ];
            $button_text = trim(strtr($button_text, $arr));
            $this->design->assign('pagination_btn_text', $button_text);
        }
        ...
        if ($ajax) {
            ...
            $result->live_pagination     = $this->design->fetch('pagination_live.tpl');
            ...
            $result->furl                = $this->filter_chpu_url(array(), $this->design);
        ...
        // Мета теги с модул фильтров, которые задаются по ссылке
        // REFACTOR: Найти нормальный способ взять текущий uri
        $currentUri  = $_SERVER['REQUEST_URI'];
        $metaRewrite = $this->products->get_metabyuri($currentUri);
        $seoComment  = $this->products->get_seocommentbyuri($currentUri);
        $fastFilters = $this->products->get_fastFiltersByUri($currentUri);
        $this->design->assign('metaRewrite', $metaRewrite);
        $this->design->assign('seoComment', $seoComment);
        $this->design->assign('fastFilters', $fastFilters);
        ...
        if ($this->category) {
            ...
            if ($metaRewrite) {
                $this->seo_filter_pattern['title']            = $metaRewrite->meta_title;
                $this->seo_filter_pattern['meta_description'] = $metaRewrite->meta_description;
            } else {
