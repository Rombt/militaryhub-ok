...
{elseif $module == 'BlogView' && $post}
    ...
    {else}
        <meta property="og:image" content="{$config->root_url}/design/{$settings->theme}/images/sroptfly-og.jpg">
        <meta name="twitter:image" content="{$config->root_url}/design/{$settings->theme}/images/sroptfly-og.jpg">
    {/if}
...
{else}
    ...
    <link rel="image_src" href="{$config->root_url}/design/{$settings->theme}/images/sroptfly-og.jpg">
    {*twitter*}
    <meta name="twitter:card" content="summary">
     ...
     {elseif $settings->captcha_type == "invisible"}
        <script>
            ...
            function onSubmitBlog(token) {
                $("#fn_blog_comment").submit();
            }

            // запускаємо підключення reCAPTCHA тільки коли юзер торкнеться поля в формі
            (function() {
                $("input, textarea").one("focus click", function() {
                    loadRecaptcha();
                });
            });
        </script>
