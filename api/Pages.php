...
class Pages extends Okay
{
    ...
    // Системные url
    public $system_pages = array('catalog', 'products', 'all-products', 'last-products', 'discounted', 'bestsellers', 'brands', 'blog', 'news', 'wishlist', 'comparison', 'cart', 'order', 'contact', 'user', '404', '');
    ...
    public function get_pages($filter = array(), $count = false)
    {
        ...
        if (isset($filter['not_url'])) {
            $where .= $this->db->placehold(' AND p.url != ?', $filter['not_url']);
        }
