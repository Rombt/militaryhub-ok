...
class Categories extends Okay {
	...
	public function get_categories( $filter = array() ) {
		...
		$category_featured_filter = '';
		if ( isset( $filter['category_featured'] ) ) {
			$category_featured_filter = $this->db->placehold( " AND c.featured = ?", (int) $filter['category_featured'] );
		}

		if ( isset( $filter['category_featured'] ) ) {
			$query = $this->db->placehold( "SELECT c.id FROM __categories c $lang_sql->join WHERE 1 $category_visible_filter $category_featured_filter ORDER BY c.position" );
			$this->db->query( $query );
			$ids = $this->db->results( 'id' );
			$result = array();
			foreach ( $ids as $id ) {
				if ( isset( $this->all_categories[ $id ] ) ) {
					$result[] = $this->all_categories[ $id ];
				}
			}
			return $result;
		}
		...
		if ( ! empty( $filter['level_depth'] ) ) {
			...
			//!!
			$level_depths = (array) $filter['level_depth'];
			$level_depth_filter = $this->db->placehold( "AND c.level_depth IN(?@)", $level_depths );
		...
		if ( ! empty( $filter['exclude_category_id'] ) ) {
			$exclude_category_id = $this->db->placehold( 'AND c.id NOT IN(?@)', (array) $filter['exclude_category_id'] );
		}
	...
	public function get_google_feed( $query = '' ) {
		$query = mb_strtolower( $query );
		$market_cats = array();
		$file = 'files/downloads/taxonomy-with-ids.en-US.txt';
		if ( file_exists( $file ) ) {
			$f = fopen( $file, 'r' );
			fgetcsv( $f, 0, '^' );
			while ( ! feof( $f ) ) {
				$line = fgetcsv( $f, 0, '^' );
				if ( empty( $query ) || strpos( mb_strtolower( $line[0] ), $query ) !== false ) {
					// @list($id, $category) = explode('-', $line[0]);
					$market_cats[] = $line[0];
				}
			}
			fclose( $f );
	...
	public function get_prom( $query = '' ) {
		$query = mb_strtolower( $query );
		$market_cats = array();
		$file = 'files/downloads/Prom.ua_categories.csv';
		if ( file_exists( $file ) ) {
			$f = fopen( $file, 'r' );
			$count = 0;
			fgetcsv( $f, 0, ';' );
			while ( ! feof( $f ) && $count < 1000 ) {
				$line = fgetcsv( $f, 0, ';' );
				if ( empty( $query ) || strpos( mb_strtolower( $line[0] ), $query ) !== false || strpos( mb_strtolower( $line[1] ), $query ) !== false
					|| strpos( mb_strtolower( $line[2] ), $query ) !== false || strpos( mb_strtolower( $line[3] ), $query ) !== false ) {
					$name = $line[0]
						. ( ! empty( $line[1] ) ? ( " > " . $line[1] ) : '' )
						. ( ! empty( $line[2] ) ? ( " > " . $line[2] ) : '' )
						. ( ! empty( $line[3] ) ? ( " > " . $line[3] ) : '' );
