...
foreach ($products as $product) {
    ...
    //!! rmbt 01.09.26 принято решение отключить все возможные скидки на Проме и продавать по максимально возможной цене
    print "\t\t\t<price>" . number_format(max((float)$price, (float)$compare_price), 2, '.', '') . "</price>\n";
    ...
    //     foreach ($features_values as $id => $features) {
    //         $values = array();

    //         $features_values_diff = reset(array_diff(array($features), $products_values[$p->product_id]));
    //         foreach ($features_values_diff as $v) {
    //             if (in_array($v->id, $products_values[$p->product_id])) {
    //                 $values[] .= $v->value;
    //             }
    //         }

    //         if (!empty($values)) {
    //             $feature = reset($features_values_diff);
    //             print "<param name=\"" . htmlspecialchars($feature->name) . "\">" . htmlspecialchars(implode('|', $values)) . "</param>";
    //         }
    //     }
    // }
        ...
        foreach ($products_values[$product->product_id] as $value_id) {
            if (isset($features_values[$value_id])) {
                ...
                if ($key && in_array($feature->translit, array($product->color, $product->size))) {
                    ...
                    if (++$products_feature_count > 2) { break;
                    }
