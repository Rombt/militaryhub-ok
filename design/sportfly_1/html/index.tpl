            ...
            <div class="header_center_content">
                <div class="header_left_wrapper">
                    ...
                    <a class="logo" href="{if $smarty.get.module=='MainView'}javascript:;{else}{$lang_link}{/if}">
                        <img src="design/{$settings->theme|escape}/images/logo.svg"
                ...
                <div class="informers">
                    ...
                    <div class="informer account_informer_wrapper">
                        {if $user}
                            {* User account *}
                            <a class="account_informer"
                               href="{$lang_link}user">{include file="svg.tpl" svgId="account_icon"}</a>
                        {else}
                            {* Login *}
                            <button class="account_informer fn_modal_auth" title="{$lang->index_login}"
                                    type="button">{include file="svg.tpl" svgId="account_icon"}</button>
                        {/if}
                    </div>
                    {* Wishlist informer *}
                    ...
                    <a class="fn_callback callback" href="#fn_callback"
                       data-language="index_back_call">{include file="svg.tpl" svgId="callback_icon"}
                            ...
                            {if $bi->image}
                               <picture>
                                    ...
                                    <source media="(max-width: 480px)" type="image/webp"
                                        {if $bi@first}
                                            srcset="{$bi->image|resize:500:200:false:$config->resized_banners_images_dir:null:null:true}"
                                        {else}
                                            data-srcset="{$bi->image|resize:500:200:false:$config->resized_banners_images_dir:null:null:true}" srcset="{$rootUrl}/design/{$settings->theme|escape}/images/xloading.gif"
                                        {/if}>

                                    {* desktop webp *}
                                    <source type="image/webp"
                                        {if $bi@first}
                                            srcset="{$bi->image|resize:1920:700:false:$config->resized_banners_images_dir:null:null:true}"
                                        {else}
                                            data-srcset="{$bi->image|resize:1920:700:false:$config->resized_banners_images_dir:null:null:true}" srcset="{$rootUrl}/design/{$settings->theme|escape}/images/xloading.gif"
                                        {/if}>

                                    {* mobile jpeg *}
                                    <source media="(max-width: 480px)" type="image/jpeg"
                                        {if $bi@first}
                                            srcset="{$bi->image|resize:500:200:false:$config->resized_banners_images_dir}"
                                        {else}
                                            data-srcset="{$bi->image|resize:500:200:false:$config->resized_banners_images_dir}" srcset="{$rootUrl}/design/{$settings->theme|escape}/images/xloading.gif"
                                        {/if}>

                                    {* desktop jpeg *}
                                    <source type="image/jpeg"
                                        {if $bi@first}
                                            srcset="{$bi->image|resize:1920:700:false:$config->resized_banners_images_dir}"
                                        {else}
                                            data-srcset="{$bi->image|resize:1920:700:false:$config->resized_banners_images_dir}" srcset="{$rootUrl}/design/{$settings->theme|escape}/images/xloading.gif"
                                        {/if}>

                                    {* fallback img *}
                                    <img
                                        {if $bi@first}
                                            src="{$bi->image|resize:1920:700:false:$config->resized_banners_images_dir}"
                                            fetchpriority="high"
                                        {else}
                                            class="lazy lazy-bg"
                                            data-src="{$bi->image|resize:1920:700:false:$config->resized_banners_images_dir}"
                        ...
                        <div id="subscribe_container">
                            ...
                            <form class="subscribe_form fn_validate_subscribe" method="post">
                                ...
                                <input class="subscribe_input" type="email" name="subscribe_email" value=""
                                       data-format="email" placeholder="{$lang->form_email}">
                    ...
                    <div class="foot rmbt-foot-right-column-item foot_line foot_contacts col-sm-6 col-lg-4">
                        ...
                        <div class="footer_contacts">
                            <div class="foot_items">
                                ...
                                <div class="foot_item">
                                    <a href="tel:{preg_replace('~[^0-9\+]~', '',$lang->company_phone_1)}"
                                       data-language="company_phone_1">{$lang->company_phone_1}</a>
                                    <a href="tel:{preg_replace('~[^0-9\+]~', '',$lang->company_phone_2)}"
                                       data-language="company_phone_2">{$lang->company_phone_2}</a>
                                </div>
                            </div>
                            <div class="foot_item">
                                {include file="svg.tpl" svgId="email_icon"}
                                <a href="mailto:{$lang->company_email}"><span
                                            data-language="company_email">{$lang->company_email}</span></a>
                            </div>
                            <div class="foot_item">
...
<div class="search" style="display: none;">
    ...
    <div class="search_container">
        ...
        <form id="fn_search" class="search_form" action="{$lang_link}all-products">
            <input class="fn_search search_input" type="text" name="keyword" value="{$keyword|escape}"
                   aria-label="search" data-language="index_search" placeholder="{$lang->index_search|escape}">
            <button class="search_button" aria-label="search"
                    type="submit">{include file="svg.tpl" svgId="search_icon"}</button>
        </form>
    </div>
</div>
...
{if $smarty.get.module == 'ProductView' || $smarty.get.module == "BlogView"}
    <link href="design/{$settings->theme|escape}/css/font-awesome.min.css{if $css_version}?v={$css_version}{/if}"
          rel="stylesheet">
...
{/if}
{* Swiper slider *}
<script src="design/{$settings->theme}/js/swiper-bundle.min.js{if $js_version}?v={$js_version}{/if}"></script>
<script src="design/{$settings->theme}/js/lazyload.min.js{if $js_version}?v={$js_version}{/if}"></script>
<script src="design/{$settings->theme}/js/mobile_menu.js{if $js_version}?v={$js_version}{/if}" defer></script>
...
{* <script src="design/{$settings->theme|escape}/js/fancybox.umd.js{if $js_version}?v={$js_version}{/if}"></script> *}

{if $smarty.get.module == 'CartView'}
    <script src="design/{$settings->theme}/js/select2.full.min.js{if $js_version}?v={$js_version}{/if}" defer></script>
    <script src="design/{$settings->theme}/js/i18n/{$language->href_lang}.js{if $js_version}?v={$js_version}{/if}" defer></script>
    <script src="design/{$settings->theme}/js/cart.js{if $js_version}?v={$js_version}{/if}" defer></script>
{/if}
...
<script src="design/{$settings->theme}/js/additional-methods.min.js{if $js_version}?v={$js_version}{/if}" defer></script>
...
{if $smarty.get.module == 'ProductView' || $smarty.get.module == "BlogView"}
    <script src="design/{$settings->theme|escape}/js/modernizr-custom.js{if $js_version}?v={$js_version}{/if}"></script>
...
<script src="design/{$settings->theme}/js/analytics.js{if $js_version}?v={$js_version}{/if}" defer></script>
{*template scripts*}
