...
class ProductAdmin extends Okay
{
    ...
    public function fetch()
    {
        ...
        if ($this->request->method('post') && !empty($_POST)) {
            ...
            $product->name       = empty($product->name) ? '' : trim($product->name);
            $product->visible    = $this->request->post('visible', 'integer');
            $product->featured   = $this->request->post('featured');
            $product->main_order = $this->request->post('main_order');
            ...
            $product->automatic_translation_done = $this->request->post('automatic_translation_done', 'integer');
            $product->bonuses                    = str_replace(',', '.', $this->request->post('bonuses', 'float'));
                ...
                if ($product->id) {
                    ...
                    if (is_array($variants)) {
                        ...
                        foreach ($variants as $index => &$variant) {
                            ...
                            $variant->promo_price = $variant->promo_price > 0 ? str_replace(',', '.', $variant->promo_price) : 0;
                            ...
                            // Удалить изображение
                            if (!empty($_POST['delete_variant_image'][$index])) {
                                $this->db->query("SELECT image FROM __variants WHERE id=?", $variant->id);
                                $filename = $this->db->result('image');
                                if (!empty($filename)) {
                                    unlink($this->config->root_dir . '/' . $this->config->original_images_dir . $filename);
                                    $this->variants->update_variant($variant->id, array('image' => null));
                                }
                            }

                            // Загрузить изображение
                            if (!empty($_FILES['variant_image']['tmp_name'][$index]) && !empty($_FILES['variant_image']['name'][$index])) {
                                $attachment_tmp_name = $_FILES['variant_image']['tmp_name'][$index];
                                $attachment_name     = $_FILES['variant_image']['name'][$index];
                                $variant_url         = $this->translit_alpha($variant->name);
                                $variant_image_name  = $product->url . (!empty($variant_url) ? ('_' . $variant_url) : '') . '_' . $this->translit($attachment_name);
                                move_uploaded_file($attachment_tmp_name, $this->config->root_dir . '/' . $this->config->original_images_dir . $variant_image_name);
                                $variant->image = $variant_image_name;
                            }
                            ...
                            if (!empty($variant->id)) {
                                ...
                                $variants_sku[$variant->id] = $variant->sku;
                            }
                        }

                        $temp_variants = [];
                        foreach ($variants as &$variant) {
                            $temp_variants[$variant->id] = $variant;
                        }
                    ...
                    if (!$this->request->post('dragged')) {                    
                        ...
                        $variants_images         = (array)$this->request->post('variants_images');
                        $current_variants_images = $this->products->get_images(array('product_id' => $product->id, 'variant_id' => $variants_ids));
                        foreach ($current_variants_images as $image) {
                            if (!in_array($image->id, $variants_images)) {
                                $this->products->delete_image($image->id);
                            }
                        }
                    }

                    // Порядок изображений вариантов
                    if (is_array($variants_images)) {
                        $i = 0;
                        foreach ($variants_images as $id) {
                            $this->products->update_image($id, array('position' => $i));
                            $i++;
                        }
                    }

                    // Загрузка изображений вариантов drag-n-drop файлов
                    $variants_images         = $this->request->post('variant_images_urls');

                    // ModulesCore/ContentEngine/ProductsImage.php //!!
                    if ($this->request->post('dragged')) {

                        if (!empty($variants_images)) {
                            foreach ($variants_ids as $i => $variant_id) {
                                if (empty($variants_images[$variant_id])) continue;
                                foreach ($variants_images[$variant_id] as $image_name) {
                                    $id_img = $this->products->add_image($product->id, $image_name, '', $variant_id);
                                    if ($id_img) {
                                        $this->products->delImagFromProd($product->id);
                                    }
                                }
                            }
                        }    
                    ...
                    $dropped_variants_images = $this->request->files('dropped_variant_images');
                    if (!empty($variants_images) && !empty($dropped_variants_images)) {
                        foreach ($variants_ids as $i => $variant_id) {
                            foreach ($variants_images[$variant_id] as $url) {
                                $key      = array_search($url, $dropped_variants_images['name'][$variant_id]);
                                $filename = (!empty($variants_sku[$variant_id]) ? ($variants_sku[$variant_id] . '_') : '')
                                    . pathinfo($dropped_variants_images['name'][$variant_id][$key], PATHINFO_FILENAME)
                                    . (($i > 1) ? ('_' . $i) : '')
                                    . '.' . pathinfo($dropped_variants_images['name'][$variant_id][$key], PATHINFO_EXTENSION);
                                if ($key !== false && $image_name = $this->image->upload_image($dropped_variants_images['tmp_name'][$variant_id][$key], $filename, $this->config->original_variants_dir)) {
                                    $this->products->add_image($product->id, $image_name, '', $variant_id);
                                }
                            }
                        }
                    }
