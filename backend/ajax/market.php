...
if($okay->managers->access('categories')) {
    ...
    switch ($module) {
        ...
        case 'search_rozetka': {
            $keyword = $okay->request->get('query');
            $keywords = explode(' ', $keyword);
            $categories = $okay->rozetka->get_items(array('keyword' => $keyword));

            $suggestions = array();
            foreach ($categories as $cats) {
                $suggestion = new stdClass();
                $suggestion->data = $cats;
                $suggestion->value = $cats->name;
                $suggestions[] = $suggestion;
            }
            $res->query = $keyword;
            $res->suggestions = $suggestions;
        ...
        case 'search_rss': {
            $keyword = $okay->request->get('query');
            $keywords = explode(' ', $keyword);
            $categories = $okay->categories->get_google_feed($keyword);
        ...
        case 'search_prom': {
            $keyword = $okay->request->get('query');
            $keywords = explode(' ', $keyword);
            $categories = $okay->categories->get_prom($keyword);
