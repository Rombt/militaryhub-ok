<?php
    ...
    $rozetka = $okay->request->get('rozetka', 'integer', 0);
    if(!empty($category_id)) {
        $features = $okay->features->get_features(array('category_id'=>$category_id));
    } elseif($rozetka) {
        $rozetkaFeatureId = (int)$okay->settings->feature_id['category_rozetka'];
        $features = [$okay->features->get_feature($rozetkaFeatureId)];
    } else {
    ...
    if(!empty($product_id)) {
        ...
        foreach($okay->features_values->get_features_values([
            'feature_id' => $rozetkaFeatureId,
            'keyword' => $keyword
        ]) as $fv) {
            $features_values[$fv->feature_id][] = $fv;
        }
