 ...
 */
    ...
    $cart = $okay->cart->get_cart($_SESSION['shopping_cart']);
    ...
    foreach($deliveries as $delivery) {
        ...
        $delivery->payment_methods = array_filter($delivery->payment_methods, function ($payment_method) use ($cart) {
            return empty((float)$payment_method->max_discount_value) || (float)$cart->total_max_discount < (float)$payment_method->max_discount_value;
        });
    }
    ...
    if (count($cart->purchases) > 0) {
        ...
        if ($okay->design->is_mobile()) {
            $result['cart_purchases'] = $okay->design->fetch('cart_purchases_mobile.tpl');
        } else {
            $result['cart_purchases'] = $okay->design->fetch('cart_purchases.tpl');
        }
        $result['cart_deliveries'] = $okay->design->fetch('cart_deliveries.tpl');
        $result['cart_payments'] = $okay->design->fetch('cart_payments.tpl');
