    ...
    public function fetch()
    {
        ...
        if ($f = $this->request->get('filter', 'string')) {
            if ($f == 'featured') {
                ...
                $filter['discounted'] = 1;
            } elseif ($f == 'no_discounted') {
                $filter['no_discounted'] = 1;
            } elseif ($f == 'main_order') {
                $filter['main_order'] = 1;
            } elseif ($f == 'visible') {
                $filter['visible'] = 1;
                ...
                $filter['feed'] = 0;
            } elseif ($f == 'in_feed_rss') {
                $filter['feed_rss'] = 1;
            } elseif ($f == 'out_feed_rss') {
                $filter['feed_rss'] = 0;
            } elseif ($f == 'without_images') {
                $filter['has_images'] = 0;
            } elseif ($f == 'instock_no_discounted') {
                $filter['in_stock'] = 1;
                $filter['no_discounted'] = 1;
            }
        ...
        if ($this->request->method('post')) {
            ...
            if (!empty($ids)) {
                switch ($this->request->post('action')) {
                    ...
                    case 'set_main_order':
                    {
                        /*Включить "избранное (фиксация)"*/
                        $this->products->update_product($ids, array('main_order' => 1));
                        break;
                    }
                    case 'unset_main_order':
                    {
                        /*Выключить "избранное (фиксация)"*/
                        $this->products->update_product($ids, array('main_order' => 0));
                        break;
                    }
                    case 'set_feed':
                    {
                    ...
                    case 'set_feed_rozetka':
                    {
                        /*Выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_rozetka=1 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'unset_feed_rozetka':
                    {
                        /*Не выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_rozetka=0 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'set_feed_epicentr':
                    {
                        /*Выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_epicentr=1 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'unset_feed_epicentr':
                    {
                        /*Не выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_epicentr=0 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'set_feed_prom':
                    {
                        /*Выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_prom=1 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'unset_feed_prom':
                    {
                        /*Не выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_prom=0 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'set_feed_rss':
                    {
                        /*Выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_rss=1 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'unset_feed_rss':
                    {
                        /*Не выгружать в фид*/
                        $this->db->query("UPDATE __variants set feed_rss=0 where product_id in(?@)", $ids);
                        break;
                    }
                    case 'delete':
                    {
