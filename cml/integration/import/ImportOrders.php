    ...
    public function import($xml_file) {
        ...
        foreach($xml->Документ as $xml_order) {
            ...
            foreach ($xml_order->Товары->Товар as $xml_product) {
                ...
                if (isset($xml_product->Скидки->Скидка) && strval($xml_product->Скидки->Скидка->УчтеноВСумме) === 'false') {
            ...
            $this->okay->db->query('UPDATE __orders SET total_price = ? WHERE id = ? LIMIT 1', $xml_order->Сумма, $order->id);
