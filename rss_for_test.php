...
$products = HelpersGlob::get_obj( $okay->db->results(), 6 );
...
foreach ( $products as $product ) {
	...
	if ( isset( $images[ $product->color ] ) ) {
		...
		// print "\t\t\t<g:image_link>" . $okay->design->resize_modifier($image, 800, 800, false, $dir) . "
		// </g:image_link>\n";
		// break;
		// }
		...
		if ( is_array( $images[ $product->color ]['urls'] ) ) {
			foreach ( $images[ $product->color ]['urls'] as $i => $image ) {
				...
				// print "\t\t\t<g:additional_image_link>" . $okay->design->resize_modifier( $image, 800, 800, false, $dir ) .
		...
		// print "\t\t\t<g:image_link>" . $okay->design->resize_modifier( $product->image, 800, 800 ) . "
		// </g:image_link>
		// \n";
	}
	...
	//!! теперь категория google обозначается только её id
	...
	// print "\t\t\t<g:google_product_category>" . trim( htmlspecialchars( $product->google_category_name ) ) . "
	// </g:google_product_category>\n";
...
echo '$general_index = ' .
	$general_index . "<br>";
echo '$index_to_feed = ' . $index_to_feed . "<br>";
exit();