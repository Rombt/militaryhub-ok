...
class ExportOrders extends Export
{
    ...
    public function export() {
        ...
        foreach ($orders as $order) {
            ...
            $bonus_paid = ($order->paid_bonuses + $order->coupon_discount);
            $order_total_price = $order->total_price + $bonus_paid;
            $order_total_price -= ($order->delivery_price * (1-$order->separate_delivery));
            $bonuses_discount = $bonus_paid / $order_total_price;
            ...
            foreach ($purchases as $purchase) {
                if (!empty($purchase->product_id) && !empty($purchase->variant_id)) {
                    ...
                    $discount = max(0, round($purchase_price * $bonuses_discount, 2));

                    $t1_2 = $t1_1->addChild ( "Наименование", $name);
                    // $t1_2 = $t1_1->addChild ( "ЦенаЗаЕдиницу", round($purchase->price*(100-$order->discount)/100));
                    $t1_2 = $t1_1->addChild ( "ЦенаЗаЕдиницу", number_format($purchase_price - $discount, 2, '.', '') );
                    $t1_2 = $t1_1->addChild ( "Количество", $purchase->amount );
                    // $t1_2 = $t1_1->addChild ( "Сумма", round($purchase->amount*$purchase->price*(100-$order->discount)/100));
                    $t1_2 = $t1_1->addChild ( "Сумма", number_format($purchase->amount * ($purchase_price - $discount), 2, '.', '') );


                    $t1_2 = $t1_1->addChild ( "Скидки" );
                    $t1_3 = $t1_2->addChild ( "Скидка" );
                    // $t1_4 = $t1_3->addChild ( "Сумма", round($purchase->amount*$purchase->price*(100-$order->discount)/100));
                    $t1_4 = $t1_3->addChild ( "Сумма", number_format($purchase->amount * ($purchase_price * (1-(100-$order->discount)/100) + $discount), 2, '.', '') );
            ...
            if ($order->delivery_price > 0 && !$order->separate_delivery) {
                ...
                $t1->addChild ( "ЦенаЗаЕдиницу", round($order->delivery_price));
                $t1->addChild ( "Количество", 1 );
                $t1->addChild ( "Сумма", round($order->delivery_price));
