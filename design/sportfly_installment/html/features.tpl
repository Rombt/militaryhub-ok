...
($category->path[$category->level_depth-2]->subcategories && $category->path[$category->level_depth-2]->count_children_visible)}
    <div class="catalog_nav">
        <div class="fn_switch filter_name">
            ...
            <span class="filter_icon">{include file="svg.tpl" svgId="filter_icon"}</span>
        </div>
        <div class="filters">
...
{*//!!*}
{if $arr_group_subcategories}
    <div class="catalog_nav">
        {foreach $arr_group_subcategories as $group_subcategories}
            {if $group_subcategories['subcategories']}
                <div class="fn_switch filter_name">
                    <span data-language="level_1">{$group_subcategories['title']}</span>
                    <span class="filter_icon">{include file="svg.tpl" svgId="filter_icon"}</span>
                </div>
                <div class="filters">
                    <div class="level_{$clearance_sale_categories->level} {if $clearance_sale_categories->level == 1}catalog_menu {else}subcatalog {/if}">
                        {foreach $group_subcategories['subcategories'] as $c}
                            {if $c->visible}
                                <div class="catalog_item has_child">
                                    <{if $c->id == $category->id}b{else}a{/if} class="catalog_link{if $c->subcategories} sub_cat{/if}{if $category->id == $c->id} selected{/if}" href="clearance-sale/sale-{$c->url}" data-category="{$c->id}">
                                        <span>{$c->name|escape}</span>
                                    </{if $c->id == $category->id}b{else}a{/if}>
                                </div>
                            {/if}
    ...
    <div class="filters">
        ...
        {if $features}
            {foreach $features as $key=>$f}
                {* Feature name *}
                <div class="fn_switch filter_name" data-feature="{$f->id}">
