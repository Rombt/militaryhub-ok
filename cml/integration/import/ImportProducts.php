...
class ImportProducts extends Import
{
    ...
    private function import_categories($xml, $parent_id = 0)
    {
        if (isset($xml->Группы->Группа)) {
            foreach ($xml->Группы->Группа as $xml_group) {
                if (strval($xml_group->Ид) === '00000000-0000-0000-0000-000000000000') {
                    $this->import_categories($xml_group, $parent_id);
                    continue;
                }
                ...
                if (in_array($name, $this->integration_1c->static_categories_name)) {
                    $this->integration_1c->set_to_storage("categories_" . strval($xml_group->Ид) . "_static", $category_id);
                }
    ...
    private function import_features($xml)
    {
        ...
        if (isset($xml->ХарактеристикиТовара->ХарактеристикаТовара)) {
            $property = $xml->ХарактеристикиТовара->ХарактеристикаТовара;
        }
        ...
        foreach ($property as $xml_feature) {
            ...
            if ($xml_feature->Наименование == $this->integration_1c->brand_option_name) {
                ...
                if (!isset($xml_feature->Ид) && !empty($xml_feature->Наименование)) {
                    $xml_feature->Ид = $xml_feature->Наименование;
                }
                // Проверяем существует ли свойство не по наименованию, а по коду 1С
                $external_id = trim(strval($xml_feature->Ид));
                $this->okay->db->query('SELECT id FROM __features WHERE external_id=?', $external_id);
                ...
                if (empty($feature_id)) {
                    ...
                    $feature_id = $this->okay->features->add_feature(array(
                        'name'        => trim(strval($xml_feature->Наименование)),
                ...
                if ($xml_feature->ТипЗначений == 'Справочник') {
                    foreach ($xml_feature->ВариантыЗначений->Справочник as $val) {
                        $value    = trim((string)$val->Значение);
                        $value_id = $this->get_feature_value_id($feature_id, $value);

                        $param = "features_values_" . $feature_id . "_" . trim(strval($val->ИдЗначения));
    ...
    private function import_product($xml_product)
    {
        ...
        if (isset($xml_product->Группы->Ид)) {
            foreach ($xml_product->Группы->Ид as $cat_id) {
                ...
                $param = "categories_" . strval($cat_id) . "_static";
                if ($this->integration_1c->get_from_storage($param) !== null) {
                    $full_update_categories = false;
                }
            }
        }
        ...
        if (isset($xml_product->ХарактеристикиТовара->ХарактеристикаТовара)) {
            foreach ($xml_product->ХарактеристикиТовара->ХарактеристикаТовара as $xml_property) {
                if (!empty($xml_property)) {
                    $param = "features_" . strval($xml_property->Наименование);
                    if ($this->integration_1c->get_from_storage($param) === null) {
                        $this->import_features($xml_product);
                    }
                }

                switch (trim(strval($xml_property->Наименование))) {
                    case 'Розмір':
                        $variant->size = strval($xml_property->Значение);
                        break;
                    case 'Колір':
                        $variant->color = strval($xml_property->Значение);
                        break;
                }
        ...
        if (empty($product_id)) {
            ...
            if ($full_update_categories === true && !empty($products_categories_ids) && !empty($product_id)) {
        ...
        // Свойства товара из ХарактеристикиТовара
        if (isset($xml_product->ХарактеристикиТовара->ХарактеристикаТовара)) {
            // Импортируем значения свойств товара
            $this->import_product_features($product_id, $main_category_id, $xml_product->ХарактеристикиТовара->ХарактеристикаТовара);
        }
    ...
    {
        $features_cleared     = [];
        $features_not_cleared = [
            $this->okay->settings->feature_id['size'],
            $this->okay->settings->feature_id['color'],
            $this->okay->settings->feature_id['category_rozetka'],
        ];
        foreach ($xml_features as $xml_option) {

            // Наименование === Ид
            if (!empty($xml_option->Наименование) && !isset($xml_option->Ид)) {
                $xml_option->Ид = $xml_option->Наименование;
            }

            $param = "features_" . trim(strval($xml_option->Ид));
            if ($this->integration_1c->get_from_storage($param) !== null) {
                $feature_id = $this->integration_1c->get_from_storage($param);

                if (isset($main_category_id) && !empty($feature_id)) {
                    if (!$features_cleared[$feature_id] && !in_array($feature_id, $features_not_cleared)) {
                        $this->okay->features_values->delete_product_value($product_id, null, $feature_id);
                        $features_cleared[$feature_id] = true;
                    }
